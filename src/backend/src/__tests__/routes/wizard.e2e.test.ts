...
describe('POST /dcfl/wizard/extract', () => {
  ...
  it('extrae contexto y devuelve 202 con jobId (fire-and-forget async)', async () => {
  ...
  it('devuelve 202 aunque el extractor falle (extract es async)', async () => {
...
// ── POST /dcfl/api/aprobaciones ─────────────────────────────────────────────
describe('POST /dcfl/api/aprobaciones', () => {
  it('devuelve 401 sin token de autenticación', async () => {
    const res = await app.request('/dcfl/api/aprobaciones', {
      method: 'POST',
...
// ── GET /dcfl/api/temario — auth requerida ───────────────────────────────────
describe('GET /dcfl/api/temario — auth requerida', () => {
  it('devuelve 401 sin Authorization header', async () => {
    const res = await app.request(`/dcfl/api/temario/${VALID_PROJECT_ID}`, {
      method: 'GET',
      headers: { 'Content-Type': 'application/json' },
    }, DEV_ENV);
    expect(res.status).toBe(401);
