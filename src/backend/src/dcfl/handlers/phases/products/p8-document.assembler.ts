...
import { parseP8Output } from '../../../helpers/renderers/p8.renderer';
...
// Detect placeholder dates inside table cells (| ... | pattern)
const PLACEHOLDER_DATE_RE = /\|\s*(TBD|PENDING|N\/A|Por definir|Pendiente|Semana\s+\d+|D[íi]a\s+\d+|DD\/MM\/YYYY|\?\?\?)\s*\|/gi;

function validateDateCoherence(doc: string): string[] {
  const issues: string[] = [];
  const isoMatches = doc.match(ISO_DATE_RE);
  if (isoMatches) {
    issues.push(`Fechas en formato ISO (YYYY-MM-DD) no convertidas: ${[...new Set(isoMatches)].slice(0, 5).join(', ')}`);
  }
  const placeholderMatches = doc.match(PLACEHOLDER_DATE_RE);
  if (placeholderMatches) {
    const unique = [...new Set(placeholderMatches.map(m => m.replace(/\|/g, '').trim()))].slice(0, 5);
    issues.push(`Fechas sin resolver en tabla: ${unique.join(', ')} — completar antes de entregar`);
  }
  return issues;
}

function appendDateWarning(doc: string, issues: string[]): string {
  return doc.trimEnd() + `
...
export async function handleDocumentP8Assembler(context: ProductContext): Promise<string> {
  ...
  // Acumular en BD — cargado aquí para que fichaFormacion esté disponible antes del Motor Gantt
  ...
  if (fichaFormacion.fecha_inicio || fichaFormacion.lugar || fichaFormacion.modalidad) {
    ...
    documentoFinal += `> ⚠️ **Sección incompleta:** No se ingresaron datos del Programa de Formación (fecha de inicio, lugar, modalidad, número de grupos). Completar estos campos antes de presentar el documento.\n\n`;
  }
  ...
  let _coberturaP8: { valido: boolean; faltantes: string[] } = { valido: true, faltantes: [] };
  try {
    ...
    _coberturaP8 = validateUnitCoverage(documentoFinal, _unis);
    if (!_coberturaP8.valido && _coberturaP8.faltantes.length > 0) {
      documentoFinal = documentoFinal.trimEnd() + `\n\n> ⚠️ **Unidades sin cobertura:** ${_coberturaP8.faltantes.join(', ')} — estas unidades del Temario Base no fueron referenciadas en este documento.`;
    }
  } catch {}

  const { doc: _p8clean, warnings: _p8sw } = sanitizeProductDocument(documentoFinal, 'P8');
  if (_p8sw.length > 0) console.warn('[p8-assembler] Sanitizer:', _p8sw);
  documentoFinal = _p8clean;

  const dateIssues = validateDateCoherence(documentoFinal);
  if (dateIssues.length > 0) {
    console.warn('[p8-assembler] ⚠️ Problemas de coherencia de fechas:', dateIssues);
    documentoFinal = appendDateWarning(documentoFinal, dateIssues);
  }

  let _anchorP8: { valido: boolean; ausentes: string[]; cobertura: number } = { valido: true, ausentes: [], cobertura: 1 };
  try {
    const briefP8 = await services.supabase.getProjectBrief(projectId);
    _anchorP8 = validateSemanticAnchor(documentoFinal, briefP8?.dominioTecnico ?? '');
    if (!_anchorP8.valido) console.warn(`[p8-assembler] ⚠ Ancla semántica: cobertura=${_anchorP8.cobertura}, ausentes=${_anchorP8.ausentes.join(', ')}`);
  } catch {}
  ...
  await services.supabase.saveF4Producto({
    ...
    datosProducto: { partes: partesAcumuladas, ficha_formacion: fichaFormacion, total_modulos: modulosOrdenados.length, validacion_cobertura: _coberturaP8, validacion_anchor: _anchorP8 },
  });
  ...
  try {
    ...
    const p8Artifact = parseP8Output(JSON.stringify(partes), nombreModulo, modalidadFrozen, idiomaReq);
    ...
    const certResult = engine.runCertificationCheck(p8Artifact, certCtx);
    ...
    console.warn('[p8-assembler] CCM saveArtifactVersion falló (no crítico):', ccmErr instanceof Error ? ccmErr.message : ccmErr);
  }
