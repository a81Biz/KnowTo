...
import { parseP3Output } from '../../../helpers/renderers/p3.renderer';
...
export async function handleDocumentP3Assembler(context: ProductContext): Promise<string> {
  ...
  let _coberturaP3: { valido: boolean; faltantes: string[] } = { valido: true, faltantes: [] };
  try {
    ...
    _coberturaP3 = validateUnitCoverage(documentoFinal, _unis);
    if (!_coberturaP3.valido && _coberturaP3.faltantes.length > 0) {
      documentoFinal = documentoFinal.trimEnd() + `\n\n> ⚠️ **Unidades sin cobertura:** ${_coberturaP3.faltantes.join(', ')} — estas unidades del Temario Base no fueron referenciadas en este documento.`;
    }
  } catch {}

  let _anchorP3: { valido: boolean; ausentes: string[]; cobertura: number } = { valido: true, ausentes: [], cobertura: 1 };
  try {
    const briefP3 = await services.supabase.getProjectBrief(projectId);
    _anchorP3 = validateSemanticAnchor(documentoFinal, briefP3?.dominioTecnico ?? '');
    if (!_anchorP3.valido) console.warn(`[p3-assembler] ⚠ Ancla semántica: cobertura=${_anchorP3.cobertura}, ausentes=${_anchorP3.ausentes.join(', ')}`);
  } catch {}

  const { doc: _p3clean, warnings: _p3sw } = sanitizeProductDocument(documentoFinal, 'P3');
  if (_p3sw.length > 0) console.warn('[p3-assembler] Sanitizer:', _p3sw);
  documentoFinal = _p3clean;
  ...
  await services.supabase.saveF4Producto({
    ...
    datosProducto: { partes: partesAcumuladas, total_modulos: modulosOrdenados.length, validacion_cobertura: _coberturaP3, validacion_anchor: _anchorP3 },
  });
  ...
  try {
    ...
    // Serialize assembled partes so parseP3Output can extract escaleta + guion_tecnico
    const p3Artifact = parseP3Output(JSON.stringify(partes), nombreVideo, modalidadFrozen, idiomaReq);
    ...
    const certResult = engine.runCertificationCheck(p3Artifact, certCtx);
    ...
    console.warn('[p3-assembler] CCM saveArtifactVersion falló (no crítico):', ccmErr instanceof Error ? ccmErr.message : ccmErr);
  }
