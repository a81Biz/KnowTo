...
export async function handleF2Assembler(params: {
  ...
  let documentoFinal = `# ESPECIFICACIONES DE ANÁLISIS Y DISEÑO\n\n`;

  if (f2Payload.perfil_ingreso_ec0366 || f2Payload.perfil_ingreso) {
    documentoFinal += `## 1. PERFIL DE INGRESO\n`;
    const p = f2Payload.perfil_ingreso_ec0366 || f2Payload.perfil_ingreso;
  ...
  // Invalidar el Temario Base si F2 cambia — el temario fue diseñado sobre la
  // estructura temática anterior y ya no es congruente con la nueva.
  try {
    await supabase.client!
      .from('temario_base')
      .update({ confirmado_por_usuario: false, confirmado_at: null })
      .eq('project_id', projectId);
    console.log('[f2-assembler] temario_base invalidado por regeneración de F2');
  } catch (err) {
    console.warn('[f2-assembler] No se pudo invalidar temario_base:', err);
  }
