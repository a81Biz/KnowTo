...
export async function handleF0Assembler(params: {
  ...
  const anclas = await getJson('agente_anclas', '{}');
  ...
  try {
    if (anclas && Object.keys(anclas).length > 0) {
      await supabase.saveProjectBrief(projectId, {
        nombreOficialCurso: anclas.nombre_oficial_curso || '',
        dominioTecnico: anclas.dominio_tecnico || '',
        resultadoCentral: anclas.resultado_central || '',
        audienciaPrimaria: anclas.audiencia_primaria || ''
      });
      console.log('[F0-ENSAMBLADOR] Anclas semánticas propuestas guardadas.');
    }
