...
export const GenerateAsyncBody = z
  .object({
    projectId:  z.string().uuid(),
    stepId:     z.string().uuid(),
    phaseId:    z.enum(['F0', 'F1', 'F2', 'F2.5', 'F3', 'F4', 'F5.1', 'F5.2', 'F6.1', 'F6.2a', 'F6.2b', 'F7', 'CLOSE', 'TEMARIO_BASE']),
    promptId:   z.enum([
      'F0', 'F1', 'F2', 'F2_5', 'F3',
      'F4_P1', 'F4_P2', 'F4_P3', 'F4_P4', 'F4_P5', 'F4_P6', 'F4_P7', 'F4_P8',
      'F4_P1_GENERATE_DOCUMENT',
...
export const routeExtract = createRoute({
  ...
  responses: {
    202: {
      description: 'Extracción iniciada en background',
      content: {
        'application/json': {
          schema: z
            .object({
            ...
            .openapi('ExtractContextAsyncResponse'),
        },
      },
    },
  },
