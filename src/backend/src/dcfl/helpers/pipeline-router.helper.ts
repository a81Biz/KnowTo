...
function buildPromptHash(templateId: string, version: string): string {
  return createHash('sha256').update(`${templateId}|${version}`).digest('hex');
}
...
export async function dispatchAgentEvent(event: PipelineEvent): Promise<string | void> {
  ...
  // A. Guardar SIEMPRE el output crudo primero (con trazabilidad de prompt — PT-085)
  const pVersion = DCFL_PROMPT_VERSIONS[promptId as keyof typeof DCFL_PROMPT_VERSIONS];
  const pHash = (promptId && pVersion) ? buildPromptHash(promptId, pVersion) : undefined;
  await services.pipelineService.saveAgentOutput(jobId, agentName, output, promptId, pVersion, pHash);
