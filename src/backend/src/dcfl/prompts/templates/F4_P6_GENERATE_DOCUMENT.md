...
pipeline_steps:
  ...
  - agent: agente_horas_A
    ...
    task: |
      ...
      GLOBAL CONSTRAINT: NEVER leave unresolved placeholders ({{variable}}, [PENDIENTE], [INSERTAR], TBD). The certification standard (if any) is in _frozen.estandar_norma — not the course name. Date format is DD/MM/YYYY — never YYYY-MM-DD.
  ...
  - agent: juez_horas
    ...
    task: |
      ...
      2. Both have total_horas ≤ 0 or total_horas > 10 (impossible session length — max 10h per session).
