...
pipeline_steps:
  ...
  - agent: agente_hitos_A
    ...
    task: |
      ...
      RESTRICCIONES_OBLIGATORIAS (verifica ANTES de escribir cualquier linea):
      - IDIOMA: Escribe EXCLUSIVAMENTE en {_frozen.idioma_requerido ?? 'espanol'}. PROHIBIDO mezclar idiomas.
      ...
      CRITICAL RULE 3: DATE FORMAT IS DD/MM/YYYY ONLY. The format YYYY-MM-DD (ISO 8601) is FORBIDDEN in every field. Output "15/05/2026" not "2026-05-15".
  ...
  - agent: agente_hitos_B
    ...
    task: |
      ...
      CRITICAL RULE 3: DATE FORMAT IS DD/MM/YYYY ONLY. The format YYYY-MM-DD is FORBIDDEN. Output "15/05/2026" not "2026-05-15".
      OUTPUT ONLY THIS JSON:
  ...
  - agent: juez_hitos
    ...
    task: |
      ...
      SELECTION CRITERIA: prefer the output with logical date order, explicit dependencies, and dates in DD/MM/YYYY format (not YYYY-MM-DD).
      VETO CRITERIA — Output "RECHAZADO" ONLY IF ALL of the following apply to BOTH A and B:
      1. Both have 0 items in the "hitos" array.
      2. Both have "tarea" fields that are empty or generic placeholders ("Task name", "Tarea").
      3. Both use YYYY-MM-DD date format instead of DD/MM/YYYY.
