...
export class WebSearchService {
  private _client: TavilyClient | null = null;
  private isProd: boolean;
  private apiKey: string;

  constructor(env: Env) {
    this.isProd = env.ENVIRONMENT === 'production';
    this.apiKey = env.TAVILY_API_KEY || process.env.TAVILY_API_KEY || '';
  ...
  private get client(): TavilyClient {
    if (!this._client) {
      if (!this.apiKey) throw new Error('No API key provided for Tavily WebSearchService.');
