    ...
    for (const step of steps) {
      ...
      if (this._isAssemblyAgent(step.agent)) {
        ...
        if (options.onAgentOutput) {
          // PT-001: re-throw assembler errors so runPipelineAsync marks the job as 'failed'.
          // A silent catch here causes the job to complete as 'success' even when the
          // assembler failed to persist (e.g. FK violation), making the failure invisible.
          let override: string | void;
          try {
            override = await options.onAgentOutput(step.agent, assembled, out);
          } catch (err) {
