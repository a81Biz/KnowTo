...
export abstract class BaseSupabaseService {
  ...
  async getF2_5Recomendaciones(projectId: string): Promise<any | null> {
    ...
    const { data, error } = await this.client
      .from('fase2_5_recomendaciones')
      .select('total_videos, duracion_promedio_minutos, frecuencia_revision, estructura_videos, actividades, metricas')
      .eq('project_id', projectId)
      .order('created_at', { ascending: false })
      .limit(1)
      .maybeSingle();
  ...
  async saveF4Producto(params: {
    ...
    // UPSERT atómico: migration 041 añade UNIQUE(project_id, producto) incondicional.
    // ignoreDuplicates:false + onConflict garantiza que siempre se sobreescriba el registro existente.
    const { data, error } = await this.client
  ...
  async countAprobaciones(projectId: string): Promise<number> {
    if (!this.client) return 0;
    const { count, error } = await this.client
      .from('expediente_aprobaciones')
      .select('id', { count: 'exact', head: true })
      .eq('project_id', projectId);
    if (error) throw new Error(`countAprobaciones failed: ${error.message}`);
    return count ?? 0;
  }

  async saveAprobacion(params: {
    projectId: string;
    folio: string;
    fase: string;
    aprobadoPor: string;
    ...
    if (!this.client) return;
    const { error } = await this.client.from('expediente_aprobaciones').insert({
      project_id: params.projectId,
      folio: params.folio,
      fase: params.fase,
      ...
      documento_md: params.documentoMd ?? null,
    });
    if (error) throw new Error(`saveAprobacion failed: ${error.message}`);
  }
  ...
  async getProjectBrief(projectId: string): Promise<Record<string, string> | null> {
    if (!this.client) return null;
    ...
    if (error) return null;
    return (data as { project_brief: Record<string, string> | null })?.project_brief ?? null;
  }

  async saveProjectBrief(projectId: string, brief: Record<string, string>): Promise<void> {
    if (!this.client) return;

    const { error } = await this.client
      .from('projects')
      .update({ project_brief: brief, updated_at: new Date().toISOString() })
      .eq('id', projectId);

    if (error) throw new Error(`saveProjectBrief failed: ${error.message}`);
  }
  ...
  async saveTemarioBase(params: {
    ...
    // El JSONB `temario` se guarda como wrapper con validaciones
    const temarioEnvelope: Record<string, unknown> = {
      modulos: params.temario,
      validacion_bloom: params.validacionBloom ?? { valido: true, violaciones: [] },
    };
    if (params.advertenciaDuracion) {
      temarioEnvelope.advertencia_duracion = params.advertenciaDuracion;
    }
    ...
    const { error } = await this.client
      ...
      .upsert({
        project_id: params.projectId,
        temario: temarioEnvelope,
        tiempos: params.tiempos,
        duracion_total_minutos: params.duracionTotal,
        total_unidades: params.totalUnidades,
        confirmado_por_usuario: false,
        updated_at: new Date().toISOString(),
      }, { onConflict: 'project_id' });

    if (error) throw new Error(`saveTemarioBase failed: ${error.message}`);
  }

  async getTemarioBase(projectId: string): Promise<{
    temario: object[];
    tiempos: object[] | null;
    duracion_total_minutos: number | null;
    total_unidades: number | null;
    confirmado_por_usuario: boolean;
    validacion_bloom: { valido: boolean; violaciones: string[] } | null;
  } | null> {
    if (!this.client) return null;

    const { data, error } = await this.client
      .from('temario_base')
      .select('temario,tiempos,duracion_total_minutos,total_unidades,confirmado_por_usuario')
      .eq('project_id', projectId)
      .maybeSingle();

    if (error) throw new Error(`getTemarioBase failed: ${error.message}`);
    if (!data) return null;

    // El campo `temario` puede ser el envelope { modulos, validacion_bloom } (formato nuevo)
    // o directamente el array de módulos (formato legado). Se normaliza aquí.
    const rawTemario = (data as any).temario;
    const modulos: object[] = Array.isArray(rawTemario) ? rawTemario : (rawTemario?.modulos ?? []);
    const validacionBloom = rawTemario?.validacion_bloom ?? null;

    return {
      temario: modulos,
      tiempos: (data as any).tiempos as object[] | null,
      duracion_total_minutos: (data as any).duracion_total_minutos as number | null,
      total_unidades: (data as any).total_unidades as number | null,
      confirmado_por_usuario: (data as any).confirmado_por_usuario as boolean,
      validacion_bloom: validacionBloom,
    };
  }

  async confirmarTemario(projectId: string, ediciones?: object): Promise<void> {
    if (!this.client) return;
    ...
    if (ediciones && typeof ediciones === 'object' && Object.keys(ediciones).length > 0) {
      // Fusionar ediciones del usuario con el temario existente
      const current = await this.getTemarioBase(projectId);
      if (current) {
        const edicionesRecord = ediciones as Record<string, unknown>;
        if (edicionesRecord.temario) updates.temario = edicionesRecord.temario;
        if (edicionesRecord.tiempos) updates.tiempos = edicionesRecord.tiempos;
      }
    }
    ...
    if (error) throw new Error(`confirmarTemario failed: ${error.message}`);
  }

  // ── Certification Canonical Model — ArtifactVersion persistence ───────────

  async saveArtifactVersion(params: {
    projectId:              string;
    productCode:            string;
    ...
    if (!this.client) throw new Error('saveArtifactVersion: Supabase client not initialised');

    const {
      projectId, productCode, artifact, documentoMd, certScore, status,
      correctionLog, derivedFromArtifactId, promptTemplateId, promptTemplateVersion,
      model, generatedBy,
    } = params;

    // Compute prompt_hash = SHA-256(template_id + '|' + template_version)
    const promptHash = (promptTemplateId && promptTemplateVersion)
      ? createHash('sha256').update(`${promptTemplateId}|${promptTemplateVersion}`).digest('hex')
      : null;
    ...
    // 4. Update fase4_productos projection columns

    const { error: deactivateErr } = await this.client
    ...
    if (deactivateErr) throw new Error(`saveArtifactVersion deactivate failed: ${deactivateErr.message}`);
    ...
    const nextVersion = ((versionRow as any)?.version ?? 0) + 1;
    ...
    if (insertErr) throw new Error(`saveArtifactVersion insert failed: ${insertErr.message}`);

    const newId = (inserted as any).id as string;

    // Update the projection columns on fase4_productos (best-effort, not critical)
    if (productCode.startsWith('P')) {
      await this.client
        .from('fase4_productos')
        .update({ datos_certificacion: artifact, active_artifact_id: newId })
        .eq('project_id', projectId)
        .eq('producto', productCode);
    }
