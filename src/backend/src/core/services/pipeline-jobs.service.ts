...
export class PipelineJobsService {
  ...
  /**
   * Espera a que un job termine usando Supabase Realtime (postgres_changes) como señal
   * primaria, con un poll HTTP cada 30 s como red de seguridad.
   *
   * Si Supabase no está disponible (dev sin BD) delega al poll simple.
   * Cada llamada a getJob lleva su propio timeout de 10 s para evitar cuelgues HTTP.
   *
   * @param jobId      UUID del job a esperar
   * @param timeoutMs  Tiempo máximo total (default 30 min)
   */
  async waitForJob(
    jobId: string,
    timeoutMs = 30 * 60 * 1_000,
  ): Promise<'completed' | 'failed' | 'timeout'> {
    // Fast-path: el job ya terminó antes de llegar aquí
    try {
      const initial = await this._getJobWithTimeout(jobId);
      if (initial?.status === 'completed') return 'completed';
      if (!initial || initial.status === 'failed') return 'failed';
    } catch { /* si la comprobación inicial falla, continuar con el wait */ }

    // Sin Supabase (dev sin BD): poll simple sobre el Map en memoria
    if (!this.supabase) {
      return this._pollJob(jobId, timeoutMs);
    }

    return new Promise<'completed' | 'failed' | 'timeout'>((resolve) => {
      let settled = false;
      const settle = (result: 'completed' | 'failed' | 'timeout') => {
        if (settled) return;
        settled = true;
        clearTimeout(timer);
        clearInterval(fallbackPoll);
        try { void this.supabase?.removeChannel(channel).catch(() => {}); } catch { /* ignore */ }
        resolve(result);
      };

      // Timeout global del run
      const timer = setTimeout(() => settle('timeout'), timeoutMs);

      // Fallback poll cada 30 s — captura eventos que Realtime pudiera perder
      const fallbackPoll = setInterval(async () => {
        try {
          const job = await this._getJobWithTimeout(jobId);
          if (job?.status === 'completed') settle('completed');
          else if (!job || job.status === 'failed') settle('failed');
        } catch { /* error de poll — esperar siguiente intervalo */ }
      }, 30_000);

      // Canal Realtime: escucha UPDATE en pipeline_jobs para este job
      const channel = this.supabase!
        .channel(`job:${jobId}`)
        .on(
          'postgres_changes',
          { event: 'UPDATE', schema: 'public', table: 'pipeline_jobs', filter: `id=eq.${jobId}` },
          (payload: any) => {
            const s: string = payload.new?.status ?? '';
            if (s === 'completed') settle('completed');
            else if (s === 'failed') settle('failed');
          },
        )
        .subscribe((status: string) => {
          if (status === 'CHANNEL_ERROR' || status === 'TIMED_OUT' || status === 'CLOSED') {
            console.warn(`[pipeline-jobs] Realtime ${status} para job ${jobId} — fallback poll activo`);
          }
        });
    });
  }

  /** getJob con timeout de 10 s para evitar cuelgues HTTP en self-hosted Supabase. */
  private async _getJobWithTimeout(jobId: string): Promise<PipelineJob | null> {
    return Promise.race([
      this.getJob(jobId),
      new Promise<null>((_, reject) =>
        setTimeout(() => reject(new Error('getJob timeout')), 10_000),
      ),
    ]);
  }

  /** Fallback de polling cuando no hay Supabase (in-memory dev store). */
