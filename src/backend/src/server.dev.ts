...
// Supabase Realtime bug en self-hosted dev: connToClose.close is not a function.
// Ocurre cuando el socket Phoenix nunca conecta (TIMED_OUT) y el objeto interno
// no es un WebSocket real. El Promise de cleanup queda huérfano dentro de
// @supabase/realtime-js y no se puede interceptar desde nuestro código.
// Solo se manifiesta en dev; en Supabase Cloud el WebSocket conecta normalmente.
process.on('unhandledRejection', (reason: unknown) => {
  const msg = reason instanceof Error ? reason.message : String(reason);
  if (msg.includes('connToClose.close')) {
    console.warn('[dev] Supabase Realtime cleanup bug suprimido (self-hosted conocido)');
    return;
  }
...
const server = serve(
  ...
  () => {
    ...
    // Limpia jobs zombie que quedaron en 'processing' tras un reinicio brusco del backend.
    // Workers puro no tiene evento de startup; esta lógica solo aplica al servidor Node.js de dev.
    void (async () => {
      try {
        const supabase = new SupabaseService(devEnv);
        const { count } = await supabase.client!
          .from('pipeline_jobs')
          .update({ status: 'failed', error: 'Backend reiniciado — job incompleto' })
          .eq('status', 'processing')
          .select('*', { count: 'exact', head: true });
        if (count && count > 0) {
          console.log(`[knowto-dev] Zombie cleanup: ${count} job(s) marcados como failed`);
        }
      } catch (err) {
        console.warn('[knowto-dev] Zombie cleanup falló (no crítico):', err);
      }
    })();
  }
);
