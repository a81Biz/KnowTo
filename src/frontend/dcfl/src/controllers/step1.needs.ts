...
class Step1NeedsController extends BaseStep {
  ...
  private async _loadAndRenderF0Context(): Promise<void> {
    ...
    // 2. Cargar brechas y anclas desde fase0_estructurado (NUEVO endpoint canónico)
    try {
      const estructUrl = buildEndpoint(ENDPOINTS.wizard.f0Estructurado(projectId));
      const estructRes = await getData<{ brechas: { mejores_practicas: string; competencia: string }, project_brief?: Record<string, string> }>(estructUrl);
      ...
      // Pre-fill Anclas Semánticas
      if (estructRes.data?.project_brief) {
        const brief = estructRes.data.project_brief;
        const inputNombre = this._container.querySelector<HTMLInputElement>('#input-nombre-oficial-curso');
        const inputDominio = this._container.querySelector<HTMLInputElement>('#input-dominio-tecnico');
        const inputResultado = this._container.querySelector<HTMLTextAreaElement>('#textarea-resultado-central');
        const inputAudiencia = this._container.querySelector<HTMLInputElement>('#input-audiencia-primaria');
        
        if (inputNombre && !inputNombre.value) inputNombre.value = brief.nombreOficialCurso || brief.nombre_oficial_curso || '';
        if (inputDominio && !inputDominio.value) inputDominio.value = brief.dominioTecnico || brief.dominio_tecnico || '';
        if (inputResultado && !inputResultado.value) inputResultado.value = brief.resultadoCentral || brief.resultado_central || '';
        if (inputAudiencia && !inputAudiencia.value) inputAudiencia.value = brief.audienciaPrimaria || brief.audiencia_primaria || '';
      }
    } catch (err) {
