...
class Step11ClosingController extends BaseStep {
  constructor() {
    super({
      ...
      uiConfig: {
        loadingText: 'Generando resumen del proceso...',
        submitText: '🎉 Finalizar y generar resumen',
        submittingText: '⏳ Generando resumen...',
        helpText:
          'Este es el último paso. Añade notas finales opcionales y pulsa el botón para generar ' +
          'el resumen cualitativo del proceso de certificación EC0366.',
      },
    });
  }
  ...
  override async mount(container: HTMLElement): Promise<void> {
    ...
    const downloadBtn = container.querySelector<HTMLButtonElement>('#btn-download-expediente');
    if (downloadBtn) {
      const f4Status = wizardStore.getState().steps[5]?.status;
      if (f4Status !== 'completed') {
        downloadBtn.disabled = true;
        downloadBtn.title = 'Genera todos los productos F4 antes de descargar el expediente.';
        const warning = document.createElement('p');
        warning.id = 'zip-warning';
        warning.className = 'text-amber-700 text-xs mt-2';
        warning.textContent = '⚠ Completa todos los productos del Paso 5 para habilitar la descarga.';
        downloadBtn.insertAdjacentElement('afterend', warning);
      } else {
        downloadBtn.addEventListener('click', () => void this._downloadExpediente());
      }
    }
  }
}
