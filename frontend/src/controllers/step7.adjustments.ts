...
import { showLoading, hideLoading, showError } from '../shared/ui';
...
class Step8AdjustmentsController extends BaseStep {
  private _formSchema: DynamicFormSchema | null = null;
  ...
  private async _loadDynamicForm(): Promise<void> {
    ...
    try {
      ...
      if (res.data?.formSchema) {
        this._formSchema = res.data.formSchema;
        this._renderDynamicForm(this._formSchema);
